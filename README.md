...
- **Sandboxed Code Execution**: LLM writes and runs Python code in isolated subprocess. Supports numpy, pandas, matplotlib, plotly, seaborn, scipy, sklearn. Interactive HTML/JS visualizations (Plotly 3D, Canvas games, simulations) directly in chat: collapsed with an "open in browser" link, expanded the embedded program with its source code. Identical pages and images appear only once per answer
...
- **Sub-Agents (delegation)**: Any agent can hand a self-contained task to a sub-agent via `delegate_task` — a fresh model run with its own context window, its own tool loop and no persona, conversation or memory of the caller. Only the sub-agent's report comes back to the main agent; the full transcript (thinking, every tool call and result) is shown collapsed in the chat bubble, so nothing runs under the radar. Plugin settings: allowed tool tiers (default: read and write data, no sending or deleting), recursion depth, optional delegation to another main agent, which then works with its identity and personality (role, working method), its model and its tools. Manager-worker pattern for long research, multi-document work and coding without flooding the main context. **Share the load, save prefill:** a main agent keeps only its everyday tools and delegates specialist work to other main agents with their persona, model and tools (e.g. Codine for code and sandbox). Measured with Qwen3.8-27B, AIfred's prompt shrinks from 31,730 to 17,449 tokens per turn, about 45 % less prefill. Everything a sub-agent produces (sandbox pages with source code, screenshots, camera images, sources) appears collapsed in the bubble at the point of delegation. See [Sub-Agents Plugin](docs/en/guides/plugins/subagent.md)
