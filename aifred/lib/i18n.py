...
class TranslationManager:
    ...
    _translations: Dict[str, Dict[str, str]] = {
        "de": {
            ...
            "zone_editor_help_threshold_label": "Auslöse-Schwelle",
            "zone_editor_help_threshold": "Anteil der Bildfläche, der sich gleichzeitig ändern muss, damit die Erkennungskette (Person/Gesicht/Alarm) überhaupt startet. NIEDRIGER = empfindlicher: auch kleine oder ferne Bewegungen triggern, aber Rauschen (Lichtwechsel, Blätter) löst öfter aus. HÖHER = träger: nur große/nahe Bewegung triggert — eine ferne Person kann dann unsichtbar bleiben. Kalibrieren mit »gemessen«: den Wert bei der Ziel-Bewegung ablesen (z.B. Person am Ende des Weges) und die Schwelle DARUNTER legen, aber über dem Ruhe-Rauschen.",
            "zone_editor_help_ego_label": "Schwenk-Erkennung (PTZ)",
            "zone_editor_help_ego": "Bewegt sich die KAMERA selbst (PTZ-Schwenk, Auto-Tracking), verschiebt sich das ganze Bild — das System erkennt diese globale Verschiebung, wirft KEINEN Alarm und lernt die neue Ansicht sofort neu (statt minutenlang alles blau zu zeigen). Der Schwellwert dafür (watch.motion_ego_shift_px in den Plugin-Settings, Standard 3 px) ist die Mindest-Bildverschiebung zwischen zwei Frames: NIEDRIGER = schon kleinste Verschiebungen gelten als Schwenk — Vorsicht: ein im Wind wackelnder Mast setzt die Erkennung dann ständig zurück (Detektor dauernd im Warmup = blind). HÖHER = nur schnelle Schwenks werden erkannt; langsame Schwenks erzeugen dann wieder Fehlalarme. Im Zweifel gewinnt immer der Alarm, nie die Schwenk-Vermutung.",
            ...
            "storage_intro": "Lokale Exporte & Sandbox-Dateien — manuell verwalten (kein Auto-Löschen).",
            "storage_kind_export": "Export",
            "storage_kind_sandbox": "Sandbox",
            "storage_open": "Öffnen",
            "storage_to_showcase": "In Showcase übernehmen",
            "storage_delete": "Löschen",
            "storage_delete_confirm": "Wirklich löschen?",
            "storage_refresh": "Aktualisieren",
            "storage_empty": "Keine verwaltbaren Dateien vorhanden.",
            "storage_select_all": "Alle auswählen",
            "storage_delete_selected": "Ausgewählte löschen",
            "storage_clear_all": "Alles löschen",
            "storage_clear_confirm": "Wirklich ALLES löschen?",
        ...
        "en": {
            ...
            "zone_editor_help_threshold_label": "Trigger threshold",
            "zone_editor_help_threshold": "Share of the image area that must change at once before the detection chain (person/face/alert) starts at all. LOWER = more sensitive: small or distant motion triggers too, but noise (light changes, leaves) fires more often. HIGHER = more sluggish: only large/near motion triggers — a distant person may stay invisible. Calibrate with »measured«: read the value during the target motion (e.g. a person at the end of the path) and set the threshold BELOW it, but above the idle noise.",
            "zone_editor_help_ego_label": "Pan detection (PTZ)",
            "zone_editor_help_ego": "When the CAMERA itself moves (PTZ pan, auto tracking), the whole image shifts — the system detects this global shift, raises NO alert and relearns the new view immediately (instead of showing everything blue for minutes). Its threshold (watch.motion_ego_shift_px in the plugin settings, default 3 px) is the minimum image shift between two frames: LOWER = even tiny shifts count as a pan — careful: a mast swaying in the wind then resets detection constantly (detector stuck in warmup = blind). HIGHER = only fast pans are detected; slow pans cause false alerts again. In doubt the alert always wins, never the pan assumption.",
            ...
            "tab_scheduler": "Scheduler",
            "tab_storage": "Storage",
            "storage_intro": "Local exports & sandbox files — manage manually (no auto-delete).",
            "storage_kind_export": "Export",
            "storage_kind_sandbox": "Sandbox",
            "storage_open": "Open",
            "storage_to_showcase": "Add to showcase",
            "storage_delete": "Delete",
            "storage_delete_confirm": "Really delete?",
            "storage_refresh": "Refresh",
            "storage_empty": "No managed files present.",
            "storage_select_all": "Select all",
            "storage_delete_selected": "Delete selected",
            "storage_clear_all": "Delete all",
            "storage_clear_confirm": "Really delete ALL?",
