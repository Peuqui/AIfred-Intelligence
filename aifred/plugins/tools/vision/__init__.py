    ...
    def get_prompt_instructions(self, lang: str) -> str:
        if lang.startswith("de"):
            return (
                ...
                "schreibst — nicht nur des ersten —, im Format "
                "`![Kamera <name> <zeit>](<image_url>)`, wobei du <name>, <zeit> "
                "und <image_url> durch die ECHTEN Werte aus genau diesem "
                "Vorkommnis ersetzt. Gib NIEMALS den Platzhalter-Text wörtlich "
                "aus (kein literales „image_url“, „HH:MM“ oder „<…>“) — immer "
                "die tatsächliche `image_url` aus dem Tool-Ergebnis. Bette nur "
                "die `image_url`s der Vorkommnisse ein, über die du tatsächlich "
                "schreibst; erfinde niemals URLs."
            )
        return (
            ...
            "EVERY happening you write about — not just the first — in the form "
            "`![Camera <name> <time>](<image_url>)`, replacing <name>, <time> and "
            "<image_url> with the REAL values from that happening. NEVER output "
            "the placeholder text literally (no literal 'image_url', 'HH:MM' or "
            "'<…>') — always the actual `image_url` from the tool result. Only "
            "embed the `image_url`s of happenings you actually describe; never "
            "invent URLs."
        )
