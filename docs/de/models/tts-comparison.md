...
- **Expressiv** (Betonung, Emotion, Persönlichkeit)
...
relativ langsam. Bestehende geklonte Stimmen müssen mit neuem Modell kompatibel sein
oder neu geklont werden.

---

## Streaming-Modi erklärt
...
- **Nachteil:** Höchste Latenz, Satzerkennung nötig
- AIfred nutzt diesen Modus aktuell mit Carry-Mechanismus für kurze Sätze
...
- **Vorteil:** Niedrige Latenz, Satzanfang hörbar bevor Satzende fertig
- **Nachteil:** Modell muss Intonation "raten" bevor Satzende bekannt

### 3. Echtes Text-Streaming (VibeVoice Realtime, MOSS-TTS-Realtime)
```
LLM streamt Wort für Wort → direkt an TTS → Audio kommt sofort
```
- Latenz = niedrigste (~80-300ms First-Packet)
- **Vorteil:** Keine Satzerkennung nötig, direktes LLM→TTS Piping
- **Nachteil:** Modell hat am wenigsten Kontext für Intonation
- **MOSS-TTS-Realtime** nutzt `push_text(delta)` API für inkrementelle Chunks
  mit KV-Cache Reuse über mehrere Turns (32K Kontext, ~40 Min.)

**Fazit:** Chunk-basiert ist nicht schlechter - nur langsamer. Die Qualität kann
sogar besser sein, weil das Modell den vollen Satzkontext hat. Für AIfred ist die
...
| **EchoTTS** | Unklar | Unklar | Unklar | Ja (beste Ähnlichkeit) | Niedrig (monoton) | Nein | ~10x | 44.1 kHz | Unklar | Diffusion | Unklar |
...
| **PocketTTS** | Klein | Unklar | Unklar | Ja (Safetensors) | Unklar | Ja (OpenAI-API) | Unklar | Unklar | CPU möglich! | Unklar | Unklar |
...
**Eigener Test:** MOSS-TTS behält den Akzent der Referenz-Sprache. Englischer Sprecher
klingt auf Deutsch "ausländisch". XTTS v2 trennt Stimm-Identität von Akzent besser
und klingt bei Cross-Lingual deutlich natürlicher.
...
Für AIfred (kurze Sätze, konversationell) können die Ergebnisse abweichen.

## Top-Kandidaten für AIfred

### 1. F5-TTS
- **Pro:** Höchste Qualität laut Community, 3 deutsche Fine-Tunes verfügbar, schnell
...
- **Contra:** Kein natives Streaming (Flow-basiert, generiert ganzen Satz auf einmal),
  CC-BY-NC Lizenz für Weights (Code ist MIT), Base-Modell nur EN+CN (DE braucht Fine-Tune),
  ~5% Artefakte an Chunk-Grenzen, Reference Audio Leakage möglich,
...
- **Deutsche Fine-Tunes:**
  - [aihpi/F5-TTS-German](https://huggingface.co/aihpi/F5-TTS-German) (HPI, BMBF-gefördert, 8x H100 Training)
...
### 2. Qwen3-TTS
- **Pro:** Deutsch nativ unterstützt, Streaming via Fork, guter Allrounder,
...
- **Pro:** Expressivste aller Modelle, 50+ Sprachen, Voice Cloning,
  75.7% Win-Rate über GPT-4o-mini-tts bei Emotionen, Apache 2.0
- **Contra:** Qualitätsschwankungen bei starker Variation, 3B braucht mehr VRAM,
  ~1.8x RTFX
...
- **Contra:** Brandneu (10.02.2026), braucht Python 3.12 + PyTorch 2.9 + Transformers 5.0
  (bleeding edge Dependencies), Community-Bewertung der Vorgängerversion war negativ
  (Artefakte), ~11.5 GB VRAM (gemessen auf RTX 3090 Ti, BF16), Cross-Lingual Voice
  Cloning schwach (behält Akzent der Referenzsprache), ~18-22s pro Satz (nicht
  streaming-geeignet)
...
- **Pro:** Echtes Text-Streaming via `push_text(delta)` - LLM-Chunks direkt als Audio,
  Multi-Turn KV-Cache Reuse (Stimmkonsistenz über Turns), 32K Kontext (~40 Min.),
...
- **Contra:** Separate Architektur (MossTTSRealtime, nicht kompatibel mit MossTTSLocal),
  braucht zusätzlichen MOSS-Audio-Tokenizer Codec (~24 kHz Output),
  SIM-Score etwas niedriger als Local (68.9% vs 73.42% EN),
  brandneu, noch nicht getestet
- **MOSS-TTS Familie (komplett):**
  | Modell | Params | Fokus | Für AIfred? |
  |--------|--------|-------|-------------|
  | MossTTSLocal | 1.7B | Beste Benchmarks, Forschung | Ja (aktuell integriert) |
  | MossTTSDelay | 8B | Produktion, Langform-Stabilität | Ja (RTX 8000, float32 ~34 GB) |
  | MossTTSRealtime | 1.7B | Streaming, Voice Agents | Ja (nächster Kandidat!) |
...
## Voice Cloning Kompatibilität

XTTS v2 Voice Clones (WAV/OGG Referenz-Audio) können **nicht** direkt in andere
Modelle übernommen werden. Jedes Modell hat sein eigenes Cloning-Format:
...
- **MOSS-TTS**: Referenz-Audio (keine Transkription nötig)
- **Voxtral TTS**: 2-3 Sekunden Referenz-Audio (Zero-Shot, Cross-Lingual)

Die **originalen Audioaufnahmen** der Stimmen können aber für jedes Modell
...
- **Pro:** Frontier-Qualität (schlägt ElevenLabs Flash v2.5), 9 Sprachen inkl. Deutsch nativ,
...
- **Contra:** ~16 GB VRAM Minimum (belegt eine komplette GPU neben dem LLM),
  ...
  müsste FP16 getestet werden), 4B Parameter sind viel für TTS
- **Status:** Zurückgestellt wegen VRAM-Bedarf. Erst relevant wenn dedizierte TTS-GPU verfügbar.
...
1. ~~**MOSS-TTS Local** testen~~ ✅ Integriert! (Docker-Container, VRAM-Reservation, gute Qualität)
...
6. **Higgs-Audio V2** testen (wenn mehr Expressivität gewünscht)
7. **Voxtral TTS** testen (wenn dedizierte TTS-GPU verfügbar, ~16 GB VRAM, Frontier-Qualität)
8. Falls keines überzeugt: XTTS v2 mit Carry-Mechanismus weiter optimieren

## Tipps aus der Community

- **Sentence Chunking** ist bei ALLEN Modellen nötig (nicht nur XTTS)
- **Kurze Sätze buffern** (wie unser Carry-Mechanismus) - universelles Problem
- **Mehrere Seeds testen** - beeinflusst Sprechweise und Stabilität
- **Silero VAD** für konsistente Stille zwischen Sätzen
- **STT-basierte Validierung** (Whisper) um fehlende Wörter zu erkennen
- **FlowHigh** kann 16kHz auf 48kHz upsamplen (relevant für Spark-TTS)
